...
from model.M2SA_with_FGCLIP import M2SAForCausalLM
from model.llava import conversation as conversation_lib
from utils.dataset3 import HybridDataset, collate_fn
from utils.dataset_with_FGCLIP import ValDataset, collate_fn1
...
def parse_args(args):
    ...
    parser.add_argument(
        "--vision-tower", default="openaiclip-vit-large-patch14", type=str
    )
    ...
    parser.add_argument("--val_json_name", default="testA_object_part.json", type=str)
    parser.add_argument("--fg_clip_mode", default="ipc", choices=["local", "ipc"], type=str)
    parser.add_argument("--fg_clip_root", type=str)
    parser.add_argument("--fg_clip_python", default="your_env/python", type=str)
    ...
    parser.add_argument("--dinov3_root", type=str)
    parser.add_argument("--dinov3_long_side", default=756, type=int)
    parser.add_argument("--dinov3_ref_topk", default=1, type=int)
    parser.add_argument("--refine_method", default="legacy", choices=["legacy", "collab_single", "collab_iterative"],
                        type=str)
    ...
    parser.add_argument("--whole_contrast_weight", default=0.5, type=float)
...
def main(args):
    ...
    model_args = {
        ...
        "whole_contrast_weight": args.whole_contrast_weight,
...
def save_pred_masks_as_png(pred_masks, image_path_name, save_dir):
    """
    将预测的掩码保存为PNG图片

    Args:
        pred_masks: list of torch.Tensor, 每个tensor形状为 [num_masks, H, W]
        image_path_name: 保存的名字,是path+sent形式
        save_dir: str, 保存目录
    """

    # 定义掩码的名称列表（顺序要与 pred_masks 对应）
    mask_names = ["gt", "origin", "ours", "whole", "part", "fused", "sam20"]

    # 遍历掩码列表
    for mask_tensor, name in zip(pred_masks, mask_names):
        # 转 numpy 并二值化
        ...
        filename = f"{name}_{image_path_name}.png"
...
def visualize_part_scoremap_viridis(
    ...
    if isinstance(score_whole, torch.Tensor):
        score_np_whole = torch.sigmoid(score_whole).float().cpu().numpy()
    else:
        score_np_whole = score_whole.astype(np.float32)

    # score_np_whole 可能形状不一致，需要 resize 到 (H, W)
    # score_np_whole 可能有多余维度，先 squeeze 成 2D
    if score_np_whole.ndim != 2:
        score_np_whole = score_np_whole.squeeze()  # 去掉所有 size=1 的维度

    # 如果 squeeze 后还不是 2D（比如真的是 3D 特征图），取第一个 channel
    if score_np_whole.ndim != 2:
        score_np_whole = score_np_whole[0]

    # resize 到 (H, W)
    if score_np_whole.shape != (H, W):
        score_np_whole = cv2.resize(
            score_np_whole.astype(np.float32), (W, H), interpolation=cv2.INTER_LINEAR
        )
    ...
    gt_rgb = np.stack([gt_vis] * 3, axis=-1)  # [H, W, 3]

    # ---- 4. viridis colormap（修复 deprecation warning）----
    cmap = matplotlib.colormaps["viridis"]

    # part score map -> RGB [H, W, 3]
    score_rgb = (cmap(score_np)[:, :, :3] * 255).astype(np.uint8)

    # whole score map -> RGB [H, W, 3]  ← 修复核心：2D -> 3D
    score_rgb_whole = (cmap(score_np_whole)[:, :, :3] * 255).astype(np.uint8)

    # ---- 5. overlay（part score 叠在原图上）----
    score_f = cmap(score_np)[:, :, :3]
    blended = np.clip((1 - alpha) * img_f + alpha * score_f, 0, 1)
    blended = (blended * 255).astype(np.uint8)

    # ---- 6. colorbar ----
    cb_w = 30
    gradient = np.linspace(1, 0, H).reshape(H, 1)
    cb_strip = np.repeat(
        (cmap(gradient)[:, 0, :3] * 255).astype(np.uint8)[:, np.newaxis, :],
        cb_w, axis=1
    )

    # ---- 7. 6联图拼接：原图 | whole score | part score | overlay | gt二值 | colorbar ----
    # 所有数组现在都是 [H, W_i, 3]，可以安全 concatenate
    canvas = np.concatenate(
        [img_np, score_rgb_whole, score_rgb, blended, gt_rgb, cb_strip], axis=1
    )
    ...
    canvas = np.concatenate([text_bar, canvas], axis=0)
    ...
    col_labels = ["original", "whole score", "part score (viridis)", "overlay", "gt mask"]
    col_starts = [0, W, W * 2, W * 3, W * 4]
    for label, cx in zip(col_labels, col_starts):
        draw.text((cx + 6, 4), label, fill=(200, 200, 200), font=font)

    max_chars = total_w // 8
    display_text = text if len(text) <= max_chars else text[:max_chars - 3] + "..."
    draw.text((6, 20), f"text: {display_text}", fill=(255, 220, 100), font=font)
