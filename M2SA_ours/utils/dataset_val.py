...
def collate_fn1(
    ...
    def create_part_whole_conversations(text_list, multiseg_inference=False):
        ...
        for text in text_list[0]:
            ...
            # 使用T5提取part和whole描述
            part_text, whole_text = extract_part_whole_with_t5(text)
            ...
            if multiseg_inference:
                # 对于多分割推理，使用part描述
                _seg = "[SEG]"
                answer = [_seg] * len([part_text])  # 这里假设只有一个part_text
            ...
            if multiseg_inference:
                # 对于多分割推理，使用whole描述
                _seg = "[SEG]"
                answer = [_seg] * len([whole_text])  # 这里假设只有一个whole_text
                answer = ', '.join(answer[:-1]) + ' and ' + answer[-1] + '.' if len(answer) > 1 else answer[0]
                conv.append_message(conv.roles[1], answer)
            else:
                conv.append_message(conv.roles[1], "[SEG].")
    ...
    def create_part_whole_conversations_batch(text_lists, multiseg_inference=False):
        """处理批量text_lists的版本"""
        all_part_conversations = []
        all_whole_conversations = []
        
        for text_list in text_lists:
            part_convs, whole_convs = create_part_whole_conversations(text_list, multiseg_inference)
            all_part_conversations.extend(part_convs)
            all_whole_conversations.extend(whole_convs)
        
        return all_part_conversations, all_whole_conversations
