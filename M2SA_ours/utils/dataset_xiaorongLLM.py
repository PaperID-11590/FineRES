...
_dict_file_path = "/data_16T/tc/huliwen/MMR-main/sencond_time/testB_object_part_dict.json"  # 默认字典文件路径
# _dict_file_path = "/data_16T/tc/huliwen/MMR-main/xiaorongLLM/testA_object_part_dict.json"  # 默认字典文件路径
...
def collate_fn1(
    ...
    #     ans = result.get("caption_refined", "")
    #     ans_whole = result.get("whole", "")
    #     ans_part = result.get("part", "")
    #     if result.get("ans", "no") != 'no':
    #         return 
    ...
    def create_part_whole_conversations(text_list, sent_ids_list=None, multiseg_inference=False, dict_file_path=None):
        ...
        for i, text in enumerate(texts_to_process):
            ...
            refined_text = get_part_whole_from_dict(lookup_key, part_whole_dict)
            
            # print(f"文本: '{text}' -> part: '{part_text}', whole: '{whole_text}'")
            
            # 生成part conversation
            if refined_text.strip():  # 只有当part_text不为空时才生成part conversation
    ...
    whole_conversation_list = create_part_whole_conversations(
        text_list
    )
    # print("part conversation:",part_conversation_list)
    # print("part conversation:",part_conversation_list)
    # print("whole conversation: ",whole_conversation_list)
    # 处理image tokens
    if use_mm_start_end:
        # 处理原始conversations
