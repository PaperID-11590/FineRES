...
_dict_file_path = "/data_16T/tc/huliwen/MMR-main/refccoo_result/refcoco+_unc_testA_dict.json"  # 默认字典文件路径
_part_whole_dict = None
def collate_fn1(
    batch, tokenizer=None, conv_type="llava_v1", use_mm_start_end=True, local_rank=-1, dict_file_path=None
):
    image_path_list = []
    images_list = []
    image_origin_list=[]
    ...
    def load_part_whole_dict(dict_file_path=None):
        ...
        if _part_whole_dict is None or dict_file_path:
    ...
    def get_part_whole_from_dict(sent_id_or_text, part_whole_dict=None):
        ...
        part_text = result.get("original_sentence", "")
        whole_text = result.get("original_sentence", "")
    ...
    def create_part_whole_conversations(text_list, sent_ids_list=None, multiseg_inference=False, dict_file_path=None):
        ...
        for i, text in enumerate(texts_to_process):
            ...
            part_text, whole_text = (text, text)
            part_text_list.append(part_text)
